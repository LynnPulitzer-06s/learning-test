...
export const useLearningTest = (parameters: {
  ...
  const decryptScoreHandle = useCallback(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        if (isStale()) {
          setMessage("⚠️ Request canceled - chain or signer changed");
          return;
        }

        setMessage("🔐 Decrypting your score using FHE...");
        ...
        setMessage(
          `🎉 Your score: ${clearScoreRef.current.clear} points!`
        );
      } finally {
  ...
  const submitExam = useCallback(
    (score: number) => {
      ...
      setMessage(`📝 Preparing to submit your score (${score} points)...`);
      ...
      const run = async () => {
        ...
        try {
          ...
          setMessage(`🔐 Encrypting your score using FHE... (this may take a moment)`);
          // is CPU-intensive (browser may freeze a little when FHE-WASM modules are loading)
          const enc = await input.encrypt();

          if (isStale()) {
            setMessage(`⚠️ Submission canceled - chain or signer changed`);
            return;
          }
          ...
          setMessage(`⏳ Waiting for transaction confirmation... (tx: ${tx.hash.slice(0, 10)}...)`);

          const receipt = await tx.wait();

          if (receipt?.status === 1) {
            setMessage(`✅ Score submitted successfully! Your exam is now recorded on-chain.`);
          } else {
            setMessage(`⚠️ Transaction completed but status unclear. Please check your score.`);
          }
  ...
  const computeTotalHandle = useCallback(() => {
    ...
    setMessage("📊 Computing encrypted total score across all students...");
    const readRunner = ethersReadonlyProvider ?? ethersSigner;
    c.calculateTotalScore()
      .then(async (tx: ethers.TransactionResponse) => {
        await tx.wait();
        setMessage("⏳ Transaction confirmed, retrieving encrypted handle...");
        ...
        setTotalHandle(h);
        setMessage(`✅ Encrypted total handle created! Ready to decrypt.`);
      })
      .catch((e: unknown) => {
        setMessage(`❌ Failed to compute total: ${String(e)}`);
  ...
  const computeAvgHandle = useCallback(() => {
    ...
    setMessage("📊 Computing encrypted aggregate for average calculation...");
    c.calculateAverageScore()
      .then(async (tx: ethers.TransactionResponse) => {
        await tx.wait();
        const h = await cRead.getLastAggregateHandle();
        setAvgHandle(h);
        setMessage("✅ Aggregate handle created! Ready to compute average.");
      })
      .catch((e: unknown) => setMessage(`❌ Failed to compute average: ${String(e)}`));
  }, [ethersSigner, ethersReadonlyProvider, learningTest.address, learningTest.abi]);
  ...
  const markEligible = useCallback(async (addresses: string[]) => {
    ...
    if (addresses.length === 0) {
      setMessage("⚠️ No addresses provided");
      return;
    }
    ...
    setMessage(`👑 Marking ${normalizedAddresses.length} student(s) as eligible for rewards...`);
    try {
      ...
      await tx.wait();
      setMessage(`✅ Successfully marked ${normalizedAddresses.length} student(s) as eligible!`);
      refreshScoreHandle();
    } catch (e: any) {
      const errorMsg = e?.reason || e?.message || String(e);
      setMessage(`❌ Failed to mark eligible: ${errorMsg}`);
  ...
  const claimReward = useCallback(async () => {
    ...
    setMessage("🎁 Claiming your Honor NFT reward...");
    try {
      const tx = await c.claimReward();
      setMessage("⏳ Waiting for transaction confirmation...");
      await tx.wait();
      setMessage("🎉 Congratulations! Honor NFT successfully claimed!");
      refreshScoreHandle();
    } catch (e) {
      setMessage(`❌ Failed to claim reward: ${String(e)}`);
    }
  }, [ethersSigner, learningTest.address, learningTest.abi, refreshScoreHandle]);
